...
function Editor({ close }: { close: () => void }) {
  ...
  return (
    <div>
      <nav className=" relative flex justify-center border-b border-[rgba(232,232,253,0.05)] editor-navbar gap-1 items-center">
        <div>
          <button
        ...
        <div className="absolute right-4">
          <button
            onClick={close}
            className="bg-white/10 rounded-xl text-white/40 font-bold text-xs w-6 h-6 flex items-center justify-center"
          >
            X
          </button>
        </div>
      ...
      <footer className="flex justify-end">
        <button
          className="bg-white/15 text-white/50 px-4 py-2 rounded-lg cursor-pointer hover:bg-white/20 transition"
          onClick={async () => {
            // send to the server
            exportJSON();
            close();
